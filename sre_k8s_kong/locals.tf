  ...
  instances = {
    ...
    private = {
      ...
      values = concat(var.private_chart_values, [
        jsonencode({proxy = {annotations = {"service.beta.kubernetes.io/aws-load-balancer-internal" = "true"}}}),
        jsonencode({proxy = {annotations = {"service.beta.kubernetes.io/azure-load-balancer-internal" = "true"}}})
      ])
